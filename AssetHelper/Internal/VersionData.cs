...
internal static class VersionData
{
    ...
    /// </summary>
    internal static Version EarliestAcceptableNonSceneCatalogVersion { get; } = Version.Parse("0.1.0");
    ...
    {
        if (!Version.TryParse(cachedPluginVersion, out Version toCheck)) return false;

        if (toCheck > Version.Parse(AssetHelperPlugin.Version)) return false;

        if (toCheck < earliest) return false;
